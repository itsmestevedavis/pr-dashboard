...
class PrBehindCount(unittest.TestCase):
    def test_parses_behind_by_from_compare(self):
        with mock.patch.object(server, "gh_run", return_value="61\n") as gh:
    ...
    def test_zero_when_up_to_date(self):
        with mock.patch.object(server, "gh_run", return_value="0\n"):
            self.assertEqual(pr_behind_count("o/r", "main", "feat"), 0)

    def test_zero_on_missing_ref_skips_gh_call(self):
        with mock.patch.object(
            server, "gh_run", side_effect=AssertionError("should not call gh")
        ):
    ...
    def test_zero_on_gh_error(self):
        with mock.patch.object(server, "gh_run", side_effect=RuntimeError("404")):
            self.assertEqual(pr_behind_count("o/r", "main", "feat"), 0)

    def test_zero_on_null_output(self):
        with mock.patch.object(server, "gh_run", return_value="null\n"):
            self.assertEqual(pr_behind_count("o/r", "main", "feat"), 0)
